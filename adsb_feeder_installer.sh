...
# Version 5.5 - Integrated connection monitor with automatic failover
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo ""
    echo "New in v5.5:"
    echo "  - Automatic failover between Tailscale and public IP"
    echo "  - Centralized IP configuration management"
    echo "  - Connection monitor with automatic recovery"
    echo ""
    echo "GitHub: https://github.com/cfd2474/TAK-ADSB-Feeder"
    exit 0
fi
...
echo ""
echo -e "${GREEN}[1/16] Creating installation directory...${NC}"
...
echo ""
echo -e "${GREEN}[3/16] Installing dependencies...${NC}"
...
echo ""
echo -e "${GREEN}[4/16] Configuring vnstat with 90-day retention...${NC}"
...
echo ""
echo -e "${GREEN}[5/16] Creating remote user...${NC}"
...
echo ""
echo -e "${GREEN}[6/16] Installing Tailscale...${NC}"
...
echo ""
echo -e "${GREEN}[7/16] Configuring RTL-SDR drivers...${NC}"
...
echo ""
echo -e "${GREEN}[8/16] Building and installing readsb...${NC}"
cd /tmp
...
echo ""
echo -e "${GREEN}[9/16] Creating readsb user and directories...${NC}"
...
echo ""
echo -e "${GREEN}[10/16] Installing tar1090 for local web interface...${NC}"
cd /tmp
...
echo ""
echo -e "${GREEN}[11/16] Creating readsb service...${NC}"
...
echo ""
echo -e "${GREEN}[12/16] Installing mlat-client...${NC}"
cd /tmp
# Force cleanup of any existing directory
...
echo ""
echo -e "${GREEN}[13/16] Creating mlat-client service...${NC}"
...
echo ""
echo -e "${GREEN}[14/16] Installing connection monitor...${NC}"
...
echo ""
echo -e "${GREEN}[15/16] Creating system update command...${NC}"
...
echo ""
echo -e "${GREEN}[16/16] Starting services...${NC}"
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  5. Test failover: Stop Tailscale and watch monitor switch to public IP"
fi
echo ""
