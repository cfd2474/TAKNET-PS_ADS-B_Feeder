...
# Version 5.6 - Added gain configuration during setup and adsb-gain command
...
# These IPs are sourced from the connection monitor script
# ============================================================================

# Default IPs (will be overridden by connection monitor if installed)
AGGREGATOR_TAILSCALE_IP="100.117.34.88"
AGGREGATOR_PUBLIC_IP="104.225.219.254"
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo ""
    echo "New in v5.6:"
    echo "  - Interactive gain configuration during setup"
    echo "  - adsb-gain command for easy gain changes"
    echo "  - Improved gain validation and range checking"
    echo ""
    echo "GitHub: https://github.com/cfd2474/TAK-ADSB-Feeder"
    exit 0
fi
...
echo -e "${GREEN}║   with Auto-Failover & Local tar1090      ║${NC}"
...
# Prompt for connection monitor installation
echo -e "${YELLOW}╔═══════════════════════════════════════════╗${NC}"
echo -e "${YELLOW}║  Connection Monitor & Failover (NEW!)     ║${NC}"
echo -e "${YELLOW}╚═══════════════════════════════════════════╝${NC}"
echo ""
echo "The connection monitor provides automatic failover to public IP"
echo "when Tailscale connection is unavailable."
echo ""
echo -e "${GREEN}Benefits:${NC}"
echo "  ✓ Automatic switch to public IP if Tailscale fails"
echo "  ✓ Automatic return to Tailscale when connection recovers"
echo "  ✓ Maximum uptime for your feeder"
echo "  ✓ Comprehensive logging of all connection events"
echo ""
echo -e "${YELLOW}Public IP fallback: $AGGREGATOR_PUBLIC_IP${NC}"
echo ""
read -p "Install connection monitor with automatic failover? (Y/n) " -n 1 -r
echo
echo ""

if [[ $REPLY =~ ^[Nn]$ ]]; then
    INSTALL_CONNECTION_MONITOR=false
    echo -e "${YELLOW}Connection monitor will not be installed${NC}"
    echo -e "${BLUE}You can install it later from GitHub${NC}"
else
    INSTALL_CONNECTION_MONITOR=true
    echo -e "${GREEN}Connection monitor will be installed${NC}"
fi
echo ""
...
# Prompt for RTL-SDR gain configuration
echo -e "${YELLOW}╔═══════════════════════════════════════════╗${NC}"
echo -e "${YELLOW}║  RTL-SDR Gain Configuration               ║${NC}"
echo -e "${YELLOW}╚═══════════════════════════════════════════╝${NC}"
echo ""
echo "RTL-SDR gain controls receiver sensitivity."
echo ""
echo -e "${GREEN}Common gain values:${NC}"
echo "  -10   = Auto gain (recommended for most setups)"
echo "  0.0   = Minimum gain"
echo "  20-30 = Typical manual values for good reception"
echo "  49.6  = Maximum gain (may cause overload in strong signal areas)"
echo ""
echo -e "${BLUE}Current default: $RTL_GAIN${NC}"
echo ""
read -p "Enter gain value (-10 for auto, or 0.0-49.6) [press Enter for default]: " GAIN_INPUT

if [ -n "$GAIN_INPUT" ]; then
    # Validate gain value
    if [[ "$GAIN_INPUT" =~ ^-?[0-9]+\.?[0-9]*$ ]]; then
        RTL_GAIN="$GAIN_INPUT"
        echo -e "${GREEN}Gain set to: $RTL_GAIN${NC}"
    else
        echo -e "${YELLOW}Invalid gain value, using default: $RTL_GAIN${NC}"
    fi
else
    echo -e "${YELLOW}Using default gain: $RTL_GAIN${NC}"
fi
echo ""
...
echo ""
echo "  Aggregator (Tailscale): $AGGREGATOR_TAILSCALE_IP"
echo "  Aggregator (Public):    $AGGREGATOR_PUBLIC_IP"
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  Failover Monitor: Enabled (auto-switch to public IP)"
else
    echo "  Failover Monitor: Disabled"
fi
...
echo ""
echo -e "${GREEN}[1/17] Creating installation directory...${NC}"
...
echo ""
echo -e "${GREEN}[2/17] Updating system packages...${NC}"
sudo apt-get update
sudo apt-get upgrade -y

echo ""
echo -e "${GREEN}[3/17] Installing dependencies...${NC}"
...
echo ""
echo -e "${GREEN}[4/17] Configuring vnstat with 90-day retention...${NC}"
...
echo ""
echo -e "${GREEN}[5/17] Creating remote user...${NC}"
...
echo "$REMOTE_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart adsb-connection-monitor" | sudo tee -a "/etc/sudoers.d/$REMOTE_USER" > /dev/null
echo "$REMOTE_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl status adsb-connection-monitor" | sudo tee -a "/etc/sudoers.d/$REMOTE_USER" > /dev/null
echo "$REMOTE_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl daemon-reload" | sudo tee -a "/etc/sudoers.d/$REMOTE_USER" > /dev/null
...
echo ""
echo -e "${GREEN}[6/17] Installing Tailscale...${NC}"
...
echo ""
echo -e "${GREEN}[7/17] Configuring RTL-SDR drivers...${NC}"
...
echo ""
echo -e "${GREEN}[8/17] Building and installing readsb...${NC}"
cd /tmp
...
echo ""
echo -e "${GREEN}[9/17] Creating readsb user and directories...${NC}"
...
echo ""
echo -e "${GREEN}[10/17] Installing tar1090 for local web interface...${NC}"
cd /tmp
...
echo ""
echo -e "${GREEN}[11/17] Creating readsb service...${NC}"
...
echo ""
echo -e "${GREEN}[12/17] Installing mlat-client...${NC}"
cd /tmp
# Force cleanup of any existing directory
...
echo ""
echo -e "${GREEN}[13/17] Creating mlat-client service...${NC}"
...
echo ""
echo -e "${GREEN}[14/17] Installing connection monitor...${NC}"

if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo -e "${BLUE}Downloading connection monitor from GitHub...${NC}"
    
    # Download connection monitor script
    if curl -fsSL "$CONNECTION_MONITOR_URL" -o /tmp/adsb-connection-monitor.sh 2>/dev/null || wget -q "$CONNECTION_MONITOR_URL" -O /tmp/adsb-connection-monitor.sh 2>/dev/null; then
        chmod +x /tmp/adsb-connection-monitor.sh
        
        # Install as system service
        sudo /tmp/adsb-connection-monitor.sh install
        
        echo -e "${GREEN}✓ Connection monitor installed and started${NC}"
        echo -e "${BLUE}  Monitor will automatically switch to public IP if Tailscale fails${NC}"
        echo -e "${BLUE}  Check status: sudo systemctl status adsb-connection-monitor${NC}"
        echo -e "${BLUE}  View logs: sudo tail -f /var/log/adsb-failover.log${NC}"
    else
        echo -e "${RED}✗ Failed to download connection monitor${NC}"
        echo -e "${YELLOW}  You can install it manually later from GitHub${NC}"
    fi
else
    echo -e "${YELLOW}Skipping connection monitor installation${NC}"
fi

echo ""
echo -e "${GREEN}[15/17] Creating gain configuration command...${NC}"

sudo tee /usr/local/bin/adsb-gain > /dev/null <<'GAINEOF'
#!/bin/bash
# ADS-B Feeder Gain Configuration Tool
# Easily change RTL-SDR gain settings
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "Usage: adsb-gain [value]"
    echo ""
    echo "Set RTL-SDR gain for readsb receiver"
    echo ""
    echo "Common values:"
    echo "  -10      Auto gain (recommended)"
    echo "  0.0      Minimum gain"
    echo "  20-30    Typical manual gain for good reception"
    echo "  49.6     Maximum gain (may cause overload)"
    echo ""
    echo "Examples:"
    echo "  adsb-gain           # Show current gain"
    echo "  adsb-gain -10       # Set auto gain"
    echo "  adsb-gain 30.0      # Set manual gain to 30.0"
    echo ""
    exit 0
fi

# Show current gain if no arguments
if [ -z "$1" ]; then
    CURRENT_GAIN=$(grep 'ExecStart.*--gain' "$SERVICE_FILE" | sed 's/.*--gain \([^ ]*\).*/\1/')
    echo -e "${BLUE}Current gain setting: ${GREEN}$CURRENT_GAIN${NC}"
    echo ""
    echo "To change gain, use: adsb-gain <value>"
    echo "For help: adsb-gain --help"
    exit 0
fi
...
if ! [[ "$NEW_GAIN" =~ ^-?[0-9]+\.?[0-9]*$ ]]; then
    echo -e "${RED}Error: Invalid gain value '$NEW_GAIN'${NC}"
    echo "Gain must be a number (-10 for auto, or 0.0-49.6)"
    exit 1
fi

# Check if value is in valid range
if (( $(echo "$NEW_GAIN < -10" | bc -l) )) || (( $(echo "$NEW_GAIN > 49.6" | bc -l) )); then
    echo -e "${YELLOW}Warning: Gain $NEW_GAIN is outside typical range${NC}"
    echo "Valid range: -10 (auto) or 0.0-49.6"
    read -p "Continue anyway? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        exit 1
...
echo -e "${BLUE}Changing RTL-SDR gain to: ${GREEN}$NEW_GAIN${NC}"

# Backup service file
sudo cp "$SERVICE_FILE" "${SERVICE_FILE}.backup.$(date +%Y%m%d_%H%M%S)"

# Update gain in service file
sudo sed -i "s/--gain [^ ]*/--gain $NEW_GAIN/" "$SERVICE_FILE"

# Reload systemd and restart readsb
echo -e "${BLUE}Reloading systemd daemon...${NC}"
sudo systemctl daemon-reload

echo -e "${BLUE}Restarting readsb service...${NC}"
...
if systemctl is-active --quiet readsb; then
    echo -e "${GREEN}✓ Gain changed successfully to $NEW_GAIN${NC}"
    echo ""
    echo -e "${BLUE}Monitor reception:${NC}"
    echo "  Web interface: http://$(hostname -I | awk '{print $1}')/tar1090/"
    echo "  View logs: sudo journalctl -u readsb -f"
    echo ""
    echo -e "${YELLOW}Tips for optimizing gain:${NC}"
    echo "  - Check tar1090 for message rate and range"
    echo "  - Look for yellow/red messages indicating signal issues"
    echo "  - Lower gain if you see many position errors"
    echo "  - Higher gain may help in rural areas with weak signals"
else
    echo -e "${RED}✗ Error: readsb failed to start${NC}"
    echo "Check logs with: sudo journalctl -u readsb -n 50"
    exit 1
fi
GAINEOF

sudo chmod +x /usr/local/bin/adsb-gain
echo -e "${GREEN}✓ Created gain configuration command: adsb-gain${NC}"

echo ""
echo -e "${GREEN}[16/17] Creating system update command...${NC}"
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo "  monitor          - Update connection monitor only"
...
# Update connection monitor if requested
if [ "$1" = "monitor" ] || [ "$1" = "all" ]; then
    if [ -f /usr/local/bin/adsb-connection-monitor ]; then
        echo -e "${YELLOW}Updating connection monitor...${NC}"
        
        # Download latest version
        TEMP_MONITOR=$(mktemp)
        if curl -fsSL "$CONNECTION_MONITOR_URL" -o "$TEMP_MONITOR" 2>/dev/null; then
            sudo systemctl stop adsb-connection-monitor 2>/dev/null || true
            sudo cp "$TEMP_MONITOR" /usr/local/bin/adsb-connection-monitor
            sudo chmod +x /usr/local/bin/adsb-connection-monitor
            sudo systemctl start adsb-connection-monitor 2>/dev/null || true
            rm -f "$TEMP_MONITOR"
            echo -e "${GREEN}✓ Connection monitor updated${NC}"
        else
            echo -e "${RED}✗ Failed to download connection monitor update${NC}"
            rm -f "$TEMP_MONITOR"
        fi
        echo ""
    else
        echo -e "${YELLOW}Connection monitor not installed, skipping...${NC}"
        echo ""
    fi
fi
...
if [ "$1" = "readsb" ] || [ "$1" = "mlat" ] || [ "$1" = "monitor" ] || [ "$1" = "all" ]; then
    ...
    systemctl is-active --quiet adsb-connection-monitor && echo -e "${GREEN}  ✓ connection monitor running${NC}" || echo -e "${YELLOW}  ⚠ connection monitor not installed${NC}"
fi
echo ""
UPDATEEOF
...
echo ""
echo -e "${GREEN}[17/17] Starting services...${NC}"
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    if systemctl is-active --quiet adsb-connection-monitor; then
        echo -e "${GREEN}✓ connection monitor is running${NC}"
    else
        echo -e "${YELLOW}⚠ connection monitor status uncertain${NC}"
    fi
fi
...
echo "  Primary (Tailscale): $AGGREGATOR_TAILSCALE_IP"
echo "  Failover (Public):   $AGGREGATOR_PUBLIC_IP"
echo "  Beast Port:          $AGGREGATOR_BEAST_PORT"
echo "  MLAT Port:           $AGGREGATOR_MLAT_PORT"
if [ "$ENABLE_MLAT" = true ]; then
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  Failover Monitor:    Enabled (automatic public IP fallback)"
else
    echo "  Failover Monitor:    Disabled"
fi
echo ""
echo -e "${YELLOW}RTL-SDR Configuration:${NC}"
echo "  Gain Setting:      $RTL_GAIN"
echo "  Change gain:       adsb-gain <value>"
echo "  View current gain: adsb-gain"
echo ""
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  Failover logs:     sudo tail -f /var/log/adsb-failover.log"
    echo "  Connection state:  cat /var/run/adsb-connection-state"
fi
echo ""
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  Update monitor:    adsb-update monitor"
fi
...
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  Check monitor:     sudo systemctl status adsb-connection-monitor"
fi
...
echo ""
echo -e "${YELLOW}RTL-SDR Control:${NC}"
echo "  View current gain: adsb-gain"
echo "  Set auto gain:     adsb-gain -10"
echo "  Set manual gain:   adsb-gain 25.0"
echo "  Get help:          adsb-gain --help"
echo ""
...
echo ""
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo -e "${YELLOW}Connection Monitor:${NC}"
    echo "  Current target:    cat /var/run/adsb-connection-state"
    echo "  View failover log: sudo tail -f /var/log/adsb-failover.log"
    echo "  Monitor status:    ./adsb-connection-monitor.sh status (if script available)"
    echo ""
fi
echo -e "${GREEN}═══════════════════════════════════════════${NC}"
echo ""
...
echo "  4. Optimize gain with: adsb-gain (check tar1090 for signal quality)"
if [ "$ENABLE_MLAT" = false ]; then
    echo "  5. To enable MLAT later: sudo systemctl enable mlat-client && sudo systemctl start mlat-client"
fi
if [ "$INSTALL_CONNECTION_MONITOR" = true ]; then
    echo "  6. Test failover: Stop Tailscale and watch monitor switch to public IP"
fi
echo ""
